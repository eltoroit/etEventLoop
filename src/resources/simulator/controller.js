...
export default class Controller {
	...
	async onChangeDemo(demoNumber) {
		...
		if (this.data.demoNumber !== demoNumber) {
			...
			// Switch demo
			let demo = await this.data.selectDemo(demoNumber);
	...
	async onChangeStep(stepNumber, isNext) {
		...
		this.cmpBoxes.forEach((cmp, key) => {
			cmp.processStep(previousStep, currentStep, isNext);
		});
	}
	...
	_log(previousStep, currentStep, isNext) {
		...
		console.groupCollapsed(`CHANGE_STEP | ${msg.clock} | ${msg.move} to ${msg.stepNumber} | ${msg.now}`, steps);
